function [minima, xatmin, history] = alg_EBOwithCMAR(Problem, opts, bounds)
%% ============EBOwithCMAR ============
% originally written by Mr.Abhishek Kumar, abhishek.kumar.eee13@iitbhu.ac.in
% best ranking method in the CEC'17 competition
...
while bsf_fit_var == Inf
    ...
    for temp=1:Par.PopSize
        fitx(temp) = fhd(x(temp,:)');
    end
...
%% main loop
while stop_con==0 && current_eval <= Par.Max_FES
    ...
    if (current_eval<Par.Max_FES)
        if rand<Probs(1)
            ...
            [EA_1, EA_1old, EA_obj1,probDE1,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq, current_eval,res_det] = ...
                ...
                archive_freq, Par.xmin, Par.xmax,  Par.n,  PS1,  current_eval, fhd,res_det,Par.Printing,Par.Max_FES, Par.Gmax, iter);
        end
    end
    %% ====================== Scout/CMAR phase ======================
    if (current_eval<Par.Max_FES)
        if   rand<Probs(2)
            [ EA_2, EA_obj2, setting,bestold,bestx,bnd,fitness,current_eval,res_det] = ...
                ...
                Par.xmin,Par.xmax,Par.n,PS2,current_eval,fhd,res_det,Par.Printing,Par.Max_FES);
%             disp([min(EA_obj2) Probs(1) Probs(2)]);
    ...
    if current_eval>0.75*Par.Max_FES
        if rand<Par.prob_ls
            ...
            [bestx,bestold,current_eval,succ] = LS2 (bestx,bestold,Par,current_eval,fhd,Par.Max_FES,Par.xmin,Par.xmax);
...
if (Par.n >= 1) && (Par.n<=10)
    Par.CS=50; %% cycle
    Par.Gmax = 2163;
elseif (Par.n >= 11) && (Par.n<=30)
    Par.CS=100; %% cycle
    Par.Gmax = 2745;
elseif (Par.n >= 31) && (Par.n<=50)
    Par.CS=150; %% cycle
    Par.Gmax = 3022;
else
    Par.CS=150; %% cycle
...
function [x, xold, fitx,prob,bestold,bestx,archive,hist_pos,memory_size, archive_f,archive_Cr,archive_T,archive_freq,current_eval,res_det ] = ...
    ...
    PopSize,  current_eval, fhd,res_det,Printing,Max_FES, G_Max, gg)
...
%% evaluate
[temp1,~] = size(ui);
fitx_new = zeros(1,temp1);
for temp = 1:temp1
    fitx_new(temp) = fhd(ui(temp,:)');
end
...
function[ x, fitx, setting,bestold,bestx,bnd,fitness,current_eval,res_det] = ...
    Scout( x, ~, prob, setting, iter,bestold,bestx,fitness,bnd,xmin,xmax,n,PopSize,current_eval,fhd,res_det,Printing,Max_FES)
...
for temp=1:PopSize
    fitness.raw(temp) = fhd(arxvalid(:,temp));
end
...
function [x,f,current_eval,succ] = LS2 (bestx,f,Par,current_eval,fhd,Max_FES,xmin,xmax)
