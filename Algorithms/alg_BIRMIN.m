function [minima, xatmin, history] = alg_BIRMIN(Problem, opts, bounds)
%--------------------------------------------------------------------------
% Function   : BIRMIN
...
%--------------------------------------------------------------------------
% [minima, xatmin, history] = BIRMIN(Problem, opts, bounds)
...
if SS.TESTflag  == 1
    VAL.perror = abs(SS.globalMIN - VAL.f_min_general);
    % if SS.globalMIN ~= 0
...
if (VAL.fMinNotImpr > 0) && (abs(VAL.f_min_general - VAL.fminloc) >...
    ...
    if SS.TESTflag == 1
        options = optimoptions('fmincon', 'Display', 'none','Algorithm','sqp',...
            'MaxFunctionEvaluations', VAL.n*1000, 'StepTolerance', 1e-20,...
...
% Have we exceeded the maxevals?
if  VAL.time >  600
    disp('Exceeded max fcn evals. Increase time'); VAL.perror = -1;
end
