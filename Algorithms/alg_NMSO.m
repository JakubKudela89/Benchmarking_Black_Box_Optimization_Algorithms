function [yBest, xBest, history] = alg_NMSO(Problem, opts, bounds)
% function [yBest, xBest, history] = NMSO(ftarget, func, dim, numEvaluations, maxRange, minRange, demo)
% Function NMSO 
...
history = [];
func = Problem.f;
ftarget = opts.globalmin + opts.tol;
numEvaluations = opts.maxevals;
dim = opts.dimension;
maxRange = max(bounds(:, 2));
minRange = max(bounds(:, 1));
...
%---------------------------------------------------
opts.NUM_FOLDS = 3;
opts.DX = 1e-10 * max(maxRange-minRange) * dim;
opts.DF = 1e-10 * dim;
...
%---------------------------------------------------
% folds and range
opts.MAX_RANGE = bounds(:, 2);
opts.MIN_RANGE = bounds(:, 1);
...
while(true)
    ...
    for f = 1 : opts.NUM_FOLDS % children process
        ...
        if (f == opts.MIDDLE_CELL && ~opts.IS_EVEN_FOLDS)
          ...
          if (toc >= 600) || (opts.numFEs >= numEvaluations)
              it = it + 1;
              history(it, 1) = it;
              history(it, 2) = opts.numFEs;
              history(it, 3) = yBest;
              history(it, 4) = toc;
              fprintf(...
                'Iter: %4i   f_min: %15.10f    time(s): %10.05f    fn evals: %8i\n',...
                it, yBest, toc, opts.numFEs);
              return;
          end
