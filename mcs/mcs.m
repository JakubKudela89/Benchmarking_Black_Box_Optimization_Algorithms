...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [xbest,fbest,xmin,fmi,ncall,ncloc,flag,history]=mcs(fcn,data,u,v,prt,smax,nf,stop,iinit,local,gamma,hess)
...
while s < smax & ncall + 1 <= nf
 it = it + 1;  
    ...
    if s == smax  % if smax is reached, a new sweep is started
        ...
        if stop(1) > 1
            if nsweep - nsweepbest >= stop(1),flag = 3; 
                return
            end
        end
        nsweep = nsweep + 1;
    end
    if ncall ~= history(uu, 2) && (mod(it,100) == 0 || ncall < 5000)
        uu = uu + 1;
        history(uu, 1) = uu;
        history(uu, 2) = ncall;
        history(uu, 3) = fbest;
