function [fevals,time,fbest] = runmcs(data, opts)
 tic
...
fcn = 'feval';
% data = fun;
% data = 'gklsMatlab';	% select test function from Jones et al. test set
		% bra  n=2   Branin
...
% bounds
u = arrayfun(@(i) getInfo.xl(i), 1:nvars)'; % left bound
v = arrayfun(@(i) getInfo.xu(i), 1:nvars)'; % right bound

fglob  = getInfo.fmin(nvars);
%globalXMIN = arrayfun(@(i) getInfo.xmin(i), 1:nvars)';
...
nf = 500000; 		% limit on number of f-calls
...
% if 0, 	% known global optimum, for tests only
  ...
  stop(1) = 1.e-2;	% run until this relative error is achieved
  stop(2) = fglob;	% known global optimum value
  stop(3) = 1.e-2;	% stopping tolerance for tiny fglob
